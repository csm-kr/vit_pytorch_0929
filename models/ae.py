...
if __name__ == '__main__':
    ...
    # h = Encoder(img_size=224, patch_size=14, dim=196).cuda()
    # g = Decoder(img_size=224, patch_size=14, dim=196).cuda()
    # z = h(x)
    # x_ = g(z)
    # print(z.size())
    # print(x_.size())
    ...
    x = torch.randn([4, 3, 32, 32]).cuda()
    ae = AutoEncoder(img_size=32, patch_size=8, dim=196, has_cls_token=False).cuda()
    z_seq, x_ = ae(x)
    print(z_seq.size())
    print(x_.size())
